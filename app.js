...
// ---- CHART INSTANCES ----
var chartCrowdTrend, chartEnergyTrend, chartSurgePred, chartEnergyZone, chartWaste;

/** @type {object} Shared chart styling defaults */
var chartDefaults = { responsive: true, maintainAspectRatio: false, plugins: { legend: { labels: { color: '#94a3b8', font: { size: 11 } } } }, scales: { x: { ticks: { color: '#64748b', font: { size: 10 } }, grid: { color: 'rgba(255,255,255,0.04)' } }, y: { ticks: { color: '#64748b', font: { size: 10 } }, grid: { color: 'rgba(255,255,255,0.04)' } } } };

/** Initialize all Chart.js chart instances. */
function initCharts() {
  var crowdCtx = document.getElementById('chartCrowdTrend');
  if (crowdCtx) {
    chartCrowdTrend = new Chart(crowdCtx, {
      ...
      options: Object.assign({}, chartDefaults, { plugins: Object.assign({}, chartDefaults.plugins, { legend: { display: false } }) })
    });
  }
  var energyCtx = document.getElementById('chartEnergyTrend');
  if (energyCtx) {
  ...
  var surgeCtx = document.getElementById('chartSurgePred');
  if (surgeCtx) {
    chartSurgePred = new Chart(surgeCtx, {
      type: 'bar',
      data: { labels: ZONES.slice(0, 8).map(function(z) { return z.name; }), datasets: [{ label: 'Current %', data: [], backgroundColor: 'rgba(59,130,246,0.6)' }, { label: 'Predicted +15 min', data: [], backgroundColor: 'rgba(245,158,11,0.6)' }] },
      options: chartDefaults
    });
  }
  var ezCtx = document.getElementById('chartEnergyZone');
  if (ezCtx) {
  ...
  var wasteCtx = document.getElementById('chartWaste');
  if (wasteCtx) {
...
function updateCharts() {
  var now = formatTimeShort(new Date());
  var totalPct = getAvgOccupancy();
  if (chartCrowdTrend) {
    chartCrowdTrend.data.labels.push(now);
    chartCrowdTrend.data.datasets[0].data.push(totalPct);
    if (chartCrowdTrend.data.labels.length > CHART_HISTORY_LENGTH) { chartCrowdTrend.data.labels.shift(); chartCrowdTrend.data.datasets[0].data.shift(); }
    chartCrowdTrend.update('none');
  }
  var energy = ENERGY_BASELINE_KWH - 140 + Math.random() * 400;
  if (chartEnergyTrend) {
    ...
    if (chartEnergyTrend.data.labels.length > CHART_HISTORY_LENGTH) { chartEnergyTrend.data.labels.shift(); chartEnergyTrend.data.datasets[0].data.shift(); chartEnergyTrend.data.datasets[1].data.shift(); }
...
window.StadiumApp = {
  ...
  generateRoute: function() {
    ...
    var rawInput = document.getElementById('navSearchInput').value;
    var input = sanitizeHTML(truncateInput(rawInput.trim()));
    var el = document.getElementById('navRouteResult');
    if (!el) return;
    if (!input) { el.innerHTML = '<div class="empty-state" role="alert">Please enter a destination.</div>'; return; }
    var toggleA = document.getElementById('accessibleRouteToggle');
    var toggleO = document.getElementById('offlineToggle');
    var accessible = toggleA ? toggleA.classList.contains('on') : false;
    var offline = toggleO ? toggleO.classList.contains('on') : false;
    var steps = accessible
  ...
  sendA11yChat: function() {
    if (!checkRateLimit()) return;
    var inputEl = document.getElementById('a11yChatInput');
    if (!inputEl) return;
    var msg = sanitizeHTML(truncateInput(inputEl.value.trim()));
    if (!msg) return;
    inputEl.value = '';
    var el = document.getElementById('a11yChatMessages');
    if (!el) return;
  ...
  planJourney: function() {
    if (!checkRateLimit()) return;
    var fromEl = document.getElementById('journeyFrom');
    var seatEl = document.getElementById('journeySeat');
    var from = sanitizeHTML(truncateInput((fromEl ? fromEl.value.trim() : '') || 'Times Square, NYC'));
    var seat = sanitizeHTML(truncateInput((seatEl ? seatEl.value.trim() : '') || 'Section 114 Row B'));
    var el = document.getElementById('journeyResult');
    if (!el) return;
    var steps = [
      '🏠 Depart from ' + from,
  ...
  sendLangChat: function() {
    if (!checkRateLimit()) return;
    var inputEl = document.getElementById('langChatInput');
    if (!inputEl) return;
    var msg = sanitizeHTML(truncateInput(inputEl.value.trim()));
    if (!msg) return;
    inputEl.value = '';
    var el = document.getElementById('langChatMessages');
    if (!el) return;
    ...
    setTimeout(function() {
      ...
      var key = Object.keys(translations).find(function(k) { return msg.toLowerCase().includes(k); });
      var resp = key ? translations[key] : { lang: 'Auto-detected', reply: 'I understand your message. Here\'s my response: The stadium information desk is available at Gate A, or I can help you with navigation, transport, accessibility, or any other service. Confidence: 0.94' };
  ...
  sendBridgeChat: function() {
    if (!checkRateLimit()) return;
    var inputEl = document.getElementById('bridgeChatInput');
    if (!inputEl) return;
    var msg = sanitizeHTML(truncateInput(inputEl.value.trim()));
    if (!msg) return;
    inputEl.value = '';
    var el = document.getElementById('bridgeChatMessages');
    if (!el) return;
    el.innerHTML += '<div class="chat-msg user" role="listitem"><div class="msg-lang">Staff (English)</div>' + msg + '</div>';
    setTimeout(function() {
      var translated = msg.replace(/gate/gi, 'puerta').replace(/please/gi, 'por favor').replace(/help/gi, 'ayuda').replace(/your/gi, 'su').replace(/the/gi, 'el').replace(/is/gi, 'está').replace(/to/gi, 'a');
  ...
  opsQuery: function() {
    if (!checkRateLimit()) return;
    var inputEl = document.getElementById('opsQueryInput');
    if (!inputEl) return;
    var query = sanitizeHTML(truncateInput(inputEl.value.trim()));
    var el = document.getElementById('opsQueryResult');
    if (!el) return;
    ...
    setTimeout(function() {
      var avgOcc = getAvgOccupancy();
      var activeCount = SimState.incidents.filter(function(i) { return i.status === 'Active'; }).length;
      var responses = {
  ...
  resolveIncident: function(id) {
    var safeId = sanitizeHTML(truncateInput(String(id)));
    var inc = SimState.incidents.find(function(i) { return i.id === safeId; });
    if (inc) {
      inc.status = 'Resolved';
...
document.addEventListener('click', function(e) {
  ...
  if (resolveBtn) {
    var incId = resolveBtn.getAttribute('data-incident-id');
    if (incId) window.StadiumApp.resolveIncident(incId);
    return;
  }
  // POI item delegation
  var poiItem = e.target.closest('.poi-item');
  if (poiItem) {
    var poiName = poiItem.getAttribute('data-poi');
...
document.addEventListener('keypress', function(e) {
  ...
  if (e.target.closest('.poi-item')) {
    var poiName = e.target.closest('.poi-item').getAttribute('data-poi');
    var navInput = document.getElementById('navSearchInput');
...
function init() {
  ...
  SimState.incidentIntervalId = setInterval(function() {
    if (SimState.incidents.filter(function(i) { return i.status === 'Active'; }).length < MAX_ACTIVE_INCIDENTS) {
      var t = INCIDENT_TEMPLATES[Math.floor(Math.random() * INCIDENT_TEMPLATES.length)];
      var newInc = {
